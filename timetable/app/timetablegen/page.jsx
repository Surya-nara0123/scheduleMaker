...
function isSwappable(
  ...
  return (
    is_free_professor(
      timetable_professors,
      timetable_classes[clas][day1][slot1][1],
      day2,
      slot2,
      clas
    ) &&
    is_free_professor(
      timetable_professors,
      timetable_classes[clas][day2][slot2][1],
      day1,
      slot1,
      clas
    )
...
export default function Table() {
  ...
  const handleEdit = (clas, c, r) => {
    ...
    for (let content of classSwaps) {
      ...
      if (content[0] == clas && content[1] == c && content[2] == tr) {
        document.getElementById("slot" + c + r).style.backgroundColor =
          "#dfdfdf";
  ...
  const swapClasses = async () => {
    ...
    let class1 = JSON.parse(
      JSON.stringify(
        timetablesForUsing[0][classSwaps[0][0]][classSwaps[0][1]][
          classSwaps[0][2]
        ]
      )
    );
    let class2 = JSON.parse(
      JSON.stringify(
        timetablesForUsing[0][classSwaps[1][0]][classSwaps[1][1]][
          classSwaps[1][2]
        ]
      )
    );
    ...
    if (class1[0].includes("Self")) {
      if (
        ...
        timetablesForUsing[0][classSwaps[1][0]][classSwaps[1][1]][
          classSwaps[1][2]
        ] = ["Self-Learning", "self_proff"];

        timetablesForUsing[1][proff1][classSwaps[0][1]][classSwaps[0][2]] = [
          temp1[0],
          classSwaps[1][0],
        ];
        ...
        let busyClass = JSON.parse(
          JSON.stringify(
            timetablesForUsing[1][
              timetablesForUsing[0][classSwaps[1][0]][classSwaps[1][1]][
                classSwaps[1][2]
              ][1]
            ][classSwaps[0][1]][classSwaps[0][2]]
          )
        );
        console.log(busyClass);
        document.getElementById("messages").innerHTML =
          `Professor is busy with ${busyClass[0]} for ${busyClass[1]} at the selected time.`;
      }
      return;
    }
    ...
    if (class2[0].includes("Self")) {
      if (
        ...
        timetablesForUsing[1][proff2][classSwaps[1][1]][classSwaps[1][2]] = [
          temp2[0],
          classSwaps[0][0],
        ];
    ...
    if (
      ...
      let proff1 = JSON.parse(
        JSON.stringify(
          timetablesForUsing[0][classSwaps[0][0]][classSwaps[1][1]][
            classSwaps[1][2]
          ][1]
        )
      );
      ...
      let busyClass1 =
        timetablesForUsing[1][busyProf1][classSwaps[1][1]][classSwaps[1][2]];
      let busyClass2 =
        timetablesForUsing[1][busyProf2][classSwaps[0][1]][classSwaps[0][2]];

      if (busyClass1 == "" && busyClass2 != "") {
      } else if (busyClass1 != "" && busyClass2 == "") {
        document.getElementById("messages").innerHTML =
          `Professor ${busyProf1} is busy with ${busyClass1[0]} for ${busyClass1[1]} at the selected time.`;
      } else if (busyClass1 != "" && busyClass2 != "") {
        document.getElementById("messages").innerHTML =
          `Professor ${busyProf1} is busy with ${busyClass1[0]} for ${busyClass1[1]} and Professor ${busyProf2} is busy with ${busyClass2[0]} for ${busyClass2[1]} at the selected times.`;
      } else {
        document.getElementById("messages").innerHTML =
          "Error Found!! Please contact maintainer DrunkenCloud on discord";
      }
    }
  };
                    ...
                    dataa == currentYear + " B_Tech " + currentSection && (
                      <div className="md:mt-6 md:ml-14 flex flex-col items-center bg-white p-4 rounded-lg">
                        ...
                        <div className="flex mt-4 ml-auto">
                          <div
                            id="messages"
                            className="flex font-bold font-mono text-1xl pt-2 text-red-500 mt-3 mr-2"
                          ></div>
                          <button
                            className="bg-purple-500 justify-center items-center p-3 rounded-lg mt-3 mr-2"
                            onClick={() => {
                              swapClasses();
                            }}
                          >
                          ...
                          {!lockedClasses.includes(dataa) ? (
                            ...
                            <button
                              ...
                              onClick={() => {
                                if (lockedClasses.includes(dataa)) {
                                  // remove the class from locked classes
                                  setLockedClasses(() => {
                                    return lockedClasses.filter(
                                      (item) => item !== dataa
                                    );
                                  });
                                }
