...
type course = [string, number, string, string]
...
function is_free_professor(timetable_professors: timeTable, proff: string, day: number, slot: number) {
...
function free_labs(timetable_labs: timeTable, course_code: string, day: number, slot1: number, slot2: number) {
    const usable = [];
...
function is_assigned_courses(classes_to_courses: coursesDetails) {
...
function lab_insert(lab_classes: coursesDetails, timetable_classes: timeTable, timetable_professors: timeTable, timetable_labs: timeTable, class_phy_cprog_lab: { [key: string]: string }) {
    ...
    for (let clas of classes) {
        ...
        for (let day = 0; day < 5; day++) {
            for (let slot = 0; slot < 8; slot++) {
                ...
                let possible_labs: { [key: string]: string[]} = {}
                ...
                if (((choice == "PHY102") && (options.includes("CSE102"))) || ((choice == "CSE102") && (options.includes("PHY102") && !clas.includes("IoT")))) {
                    ...
                    let hours = 0;
                    let phy_course: course = ["", 1, "", ""];
                    let cpo_course: course = ["", 1, "", ""];
                    for (let lab_course of lab_classes[clas]) {
                        if (lab_course[0] === "PHY102") {
                            hours = lab_course[1]
                            phy_course = JSON.parse(JSON.stringify(lab_course))
                        } else if (lab_course[0] === "CSE102") {
                            cpo_course = JSON.parse(JSON.stringify(lab_course))
                        }
                    }
                ...
                if (choice == "CSE102" && clas.includes("IoT")) {
                    ...
                    let course_details: course = ["", 1, "", ""];
                    for (let lab_course of lab_classes[clas]) {
                        if (lab_course[0] == choice) {
                            course_details = JSON.parse(JSON.stringify(lab_course))
                            break;
                        }
                    }
...
function theory_insert(theory_classes: coursesDetails, timetable_classes: timeTable, timetable_professors: timeTable) {
...
function class_swap(timetable_classes: timeTable, clas: string, day: number, slot: number) {
    for (let i = 0; i < 5; i++) {
        for (let j = 0; j < 8; j++) {
...
function theory_update_1(theory_classes: coursesDetails, timetable_classes: timeTable, timetable_professors: timeTable) {
    for (let clas of Object.keys(timetable_classes)) {
        ...
        for (let course of temp[clas]) {
            // Adds all left over self learning classes
            if (!course[0].includes("Self-Learning")) {
                continue;
            }
            let check = false;
...
function theory_update_2(theory_classes: coursesDetails, all_theory_classes: coursesDetails, timetable_classes: timeTable, timetable_professors: timeTable) {
        ...
        for (let day = 0; day < 5; day++) {
            for (let slot = 0; slot < 8; slot++) {
                if (timetable_classes[clas][day][slot] === "") {
                    ...
                    let replaceable: [course, course, number, number][] = []
...
function initialise_class_courses(class_courses: coursesDetails, locked_classes: string[]) {
    let result: coursesDetails = JSON.parse(JSON.stringify(class_courses));

    for (let [clas, courses] of Object.entries(result)) {
        for (let i = 0; i < courses.length; i++) {
            let course = courses[i];
            if (course[2] === "LT") {
                let temp_1: course= [course[0] + " T", course[1] - 2, "T", course[3]];
                course[1] = 2;
                course[2] = "L";
                result[clas].push(temp_1);
            } else if (course[2] === "L") {
                course[1] = course[1] - 1;
                let temp_1: course = [course[0] + " T", 1, "T", course[3]];
                result[clas].push(temp_1);
            }
        }
    }

    let temp_2: coursesDetails = JSON.parse(JSON.stringify(result));
...
// its in the name
function initialise_timetables(classes_to_courses: coursesDetails, professors: string[], labs: string[], initial_lectures: [string, string, string, number, number], locked_classes: string[], proffs_initial_timetable: timeTable, classes_initial_timetable: timeTable): [timeTable, timeTable, timeTable, { [key: string]: string}] {
    let timetable_classes_ini: timeTable = {}
    let timetable_professors_ini: timeTable = {}
    let timetable_labs_ini:  timeTable = {}
    ...
    for (let proff of professors) {
        timetable_professors_ini[proff] = Array(5).fill(null).map(() => Array(8).fill("") as [string | string[]]);
    }
    timetable_professors_ini["self_proff"] = Array(5).fill(null).map(() => Array(8).fill("") as [string | string[]]);

    for (let lab of labs) {
        timetable_labs_ini[lab] = Array(5).fill(null).map(() => Array(8).fill("") as [string | string[]]);
    }

    // For professors who teach accross 1st and 2nd eyar, they have 2 Lunchs as I dont want to deal with the logic otherwise
    let proff_to_year: { [key: string]: string} = {}
    ...
    let temp: [string, string, string, number, number][] = JSON.parse(JSON.stringify(initial_lectures))
...
function format_timetables(timetable_classes: timeTable, timetable_professors: timeTable, timetable_labs: timeTable, proff_to_year: { [key: string]: string }, proff_to_short: { [key: string]: string }){
    for(let clas of Object.keys(timetable_classes)){
        if(clas.slice(0,1) === "1"){
            for(let day = 0; day < 5; day++){
        ...
        if(proff_to_year[prof] === "1"){
            ...
            for(let day = 0; day < 5; day++){
                if(timetable_professors[prof][day][2] === ""){
                    timetable_professors[prof][day].splice(2, 0, "Break")
                } else if (timetable_professors[prof][day].length >= 2 && Array.isArray(timetable_professors[prof][day][2]) && timetable_professors[prof][day][2][1].slice(0,1) === "1"){
                    timetable_professors[prof][day].splice(2, 0, "Break")
                } else {
                    timetable_professors[prof][day].splice(3, 0, "Break")
                }
...
function verify_everything(classes_to_courses: coursesDetails, timetable_classes: timeTable, timetable_professors: timeTable, timetable_labs: timeTable) {
...
// Its in the name
function get_replacements(classes_to_courses: coursesDetails, timetable_professors: timeTable){
    let proff_replacements: {[key: string]: { [key: string]: string[]}} = {}
    for(let prof of Object.keys(timetable_professors)){
...
function get_timetables(class_courses: coursesDetails, professors: string[], proff_to_short: { [key: string]: string }, labs: string[], initial_lectures: [string, string, string, number, number], locked_classes: string[], proffs_initial_timetable: timeTable, classes_initial_timetable: timeTable) {
    ...
    while (!check && fallback < 200) {
        let classes_to_courses1: coursesDetails = JSON.parse(JSON.stringify(classes_to_courses));
        let classes_to_courses2: coursesDetails = JSON.parse(JSON.stringify(classes_to_courses));
        ...
        let lab_classes: coursesDetails = {};
        ...
        let theory_classes: coursesDetails = {};
        for (let [clas, courses] of Object.entries(classes_to_courses1)) {
...
export function randomize(class_courses: coursesDetails, professors: string[], proff_to_short: { [key: string]: string }, labs: string[], initial_lectures: [string, string, string, number, number], locked_classes: string[], proffs_initial_timetable: timeTable, classes_initial_timetable: timeTable) {
    try {
