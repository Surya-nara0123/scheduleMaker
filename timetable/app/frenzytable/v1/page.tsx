        ...
        <div className="">
          ...
          <div className="mt-12 ml-12 flex flex-col items-center bg-blue-50 p-4 rounded-lg">
            <div className="flex items-center mb-4 rounded-lg px-2">
              ...
              <div className="flex gap-1 bg-[#bfc0c0] rounded-lg">
                {["1", "2", "3", "4", "Break", "5", "6", "7"].map(
                  (slot, index) => (
                    <div
                      key={index}
                      className="w-24 h-16 flex items-center justify-center border-r-0 border "
                    >
                      {slot}
                    </div>
                  )
            ...
            <div className="flex">
              <div className="flex flex-col gap-1 bg-[#bfc0c0] rounded-lg mr-1">
                {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map(
                  (day, index) => (
                    <div
                      key={index}
                      className="w-24 h-16 flex items-center justify-center border border-b-0"
                    >
                      {day}
                    </div>
                  )
              ...
              <div className="grid grid-cols-8 grid-rows-6 bg-[#dfdfdf] rounded-lg gap-1">
                {Array.from({ length: 6 }).map((_, rowIndex) => (
                  <React.Fragment key={rowIndex}>
                    {Array.from({ length: 8 }).map((_, colIndex) => (
                      <div
                        key={colIndex}
                        className="w-24 h-16 border border-inherit border-t-0 border-l-0"
                      ></div>
                    ))}
                  </React.Fragment>
                ))}
